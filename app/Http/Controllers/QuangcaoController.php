...
class QuangcaoController extends Controller
{
    ...
    public function bieudochiphi(){
      ...
      ->select(array(DB::Raw('sum(w.sotienchay) as tongchiphi'),DB::Raw('sum(w.sodienthoai) as tongsodienthoai'),DB::Raw('sum(w.sochuyendoi) as sochuyendoi'),DB::Raw('DATE(w.ngaychay) day'),DB::Raw('sum(w.comment) as socomment'),DB::Raw('sum(w.tinnhan) as sotinnhan'),DB::Raw('sum(w.sodienthoai) as sodienthoai'),DB::Raw('sum(w.chat) as sochat')))
      ->groupBy('day')
      ->orderBy('w.ngaychay')
      ->get();
      $thongkethanhtoan = DB::table('thanhtoankhachhang as w')
      ->select(array(DB::Raw('sum(w.tongtien) as tongtientrongngay'),DB::Raw('DATE(w.updated_at) day')))
      ->groupBy('day')
      ->orderBy('w.updated_at')
      ->get();
      $thongketong = [];
