...
createImageAnhDaiDien(file){
 ...
 reader.readAsDataURL(file);
  ...
  render () {
    ...
    const { nguongioithieulist,khuvucselect,benhlyds } = this.state;
...
const { bacsilist,ghichutext } = this.state;
    ...
    <p>{moment(khachhang.created_at).format("DD-MM-YYYY HH:mm:ss")}</p>
                ...
                <div className="col-md-4"><strong>Mã giới thiệu</strong>
                  <p>{khachhang.sosao}</p>
                </div>
                <hr />
    ...
    <p>{tiendoanhthukhach.doanhthukhachhang} VNĐ</p>
              ...
              {localStorage.getItem('userrole')==="1" ? <button type="button" className="btn btn-block  btn-success" data-toggle="modal" data-target="#exampleModal2" onClick={this.updatekhachhang} data-whatever="@mdo">CHỈNH SỬA THÔNG TIN KHÁCH HÀNG</button> : ''}
              {localStorage.getItem('userrole')==="2" ? <button type="button" className="btn btn-block  btn-success" data-toggle="modal" data-target="#exampleModal2" onClick={this.updatekhachhang} data-whatever="@mdo">CHỈNH SỬA THÔNG TIN KHÁCH HÀNG</button> : ''}
              {localStorage.getItem('userrole')==="4" ? <button type="button" className="btn btn-block  btn-success" data-toggle="modal" data-target="#exampleModal2" onClick={this.updatekhachhang} data-whatever="@mdo">CHỈNH SỬA THÔNG TIN KHÁCH HÀNG</button> : ''}
              {localStorage.getItem('userrole')==="5" ? <button type="button" className="btn btn-block  btn-success" data-toggle="modal" data-target="#exampleModal2" onClick={this.updatekhachhang} data-whatever="@mdo">CHỈNH SỬA THÔNG TIN KHÁCH HÀNG</button> : ''}
 ...
 <label htmlFor={'flat-checkbox-'+ts.id}>{ts.ten}</label>
    ...
    <div className="modal-dialog" role="document">
      <div className="modal-content">
      <BangInThongTin key={khachhang.ID} benhlyds={benhlyds} benhly={benhly} datakhachhang={khachhang} dichvudieutri={dichvudieutrisingle} tiensubenh={tiensubenhsingle} nguongioithieu={nguongioithieusingle.nguon} />
