        ...
        <table className="table-bordered table-hover table color-table primary-table" >
          ...
          {dichvu.map(cd => ( 
            <tr id={"dichvuitem"+cd.id} data-itemngt={cd.id}>
              ...
              {localStorage.getItem('userrole')==="1" ? <button data-iddichvu={cd.id} onClick={this.handleDeleteDichVu} className="icon-list-demo btn btn-danger btn-circle btn-xl" data-id={cd.id}><i className="fa fa-trash-o" data-iddichvu={cd.id}></i></button> : ''}
              {localStorage.getItem('userrole')==="1" ? <button onClick={this.handleChiTietDichVu} className="icon-list-demo btn btn-info btn-circle btn-xl" data-iddichvu={cd.id} data-toggle="modal" data-target="#exampleModal1" data-whatever="@mdo"><i className="fa fa-pencil" data-iddichvu={cd.id}></i></button> : ''} 
                </td>
